...
const btnBase = {
  ...
  borderRadius: '16px', minHeight: '64px', transition: 'transform 0.2s, background 0.2s',
    ...
    <div style={{ animation: 'fadeIn 0.4s ease' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
        {options.map(opt => {
          ...
          const isCorrect = showResult && opt.value === question.answer;
          let bg = 'rgba(255,255,255,0.08)';
          let border = '2px solid rgba(255,255,255,0.15)';
          if (showResult) {
            if (isCorrect) { bg = 'rgba(78,205,196,0.2)'; border = '2px solid #4ECDC4'; }
            else if (isSelected && !result?.correct) { bg = 'rgba(255,107,107,0.2)'; border = '2px solid #FF6B6B'; }
          } else if (isSelected) {
            bg = 'rgba(249,115,22,0.2)'; border = '2px solid #F97316';
          }
          return (
      ...
      {showResult && (
        <div style={{
          ...
          border: `1px solid ${result?.correct ? 'rgba(78,205,196,0.3)' : 'rgba(255,107,107,0.3)'}`,
